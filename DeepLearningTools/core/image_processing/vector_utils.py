...
class VectorUtils:
    ...
    statDict = {
        ...
        "find_feature_shape_complexity_index": lambda geom: find_feature_shape_complexity_index(
            geom
        ),
        "equivaent_rectangular_index": lambda geom: find_equivalent_rectangular_index(
            geom
        ),
...
def find_vibration_frequency(geom):
    ...
    feature_notches_normalized = (
        float(feature_notches) / float(feature_vertices - 3)
        if feature_vertices != 3
        else 0
    )
    return (
        16.0 * (feature_notches_normalized - 0.5) ** 4
        - 8 * (feature_notches_normalized - 0.5) ** 2
        - 1.0
    )
